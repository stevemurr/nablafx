...
if __name__ == "__main__":
    ...
    model_id = ["GB-C-DIST-MLP"]
    num_controls = [4]

    for mid, nc in zip(model_id, num_controls):

        for N in frame_sizes:
            print()
            print(f"{mid}, {nc}, frame size: {N}")
            rtf = run(
                num_controls=nc,
                N=N,
            )
            # if c:   mid += "-C"
            # else:   mid += "-N"
            candidates.append({"model_id": mid, "num_controls": nc, "rtf": rtf, "N": N})

    df = pd.DataFrame(candidates)
    df.to_csv("tests/speed_cpu_gb-dist-mlp-cond.csv")
