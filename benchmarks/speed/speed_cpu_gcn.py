...
if __name__ == "__main__":
    ...
    model_id = ["GCN-45-S-16", "GCN-45-L-16", "GCN-250-S-16", "GCN-250-L-16", "GCN-2500-S-16", "GCN-2500-L-16"]
    ...
    for mid, b, d, k, ch in zip(model_id, num_blocks, dilation_factors, kernels, channel_width):
        ...
        for c, N in product(causal, frame_sizes):
            print()
            print(f"{mid}: {b}, {d}, {k}, {ch}, frame size: {N}")
            rf, rtf = run(
                num_blocks=b,
                dilation_growth=d,
                kernel_size=k,
    ...
    df.to_csv("tests/speed_cpu_gcn.csv")
