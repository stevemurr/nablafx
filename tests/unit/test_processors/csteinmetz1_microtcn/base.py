...
class Base(pl.LightningModule):
    ...
    def __init__(self, lr=3e-4, train_loss="l1+stft", save_dir=None, num_examples=4, **kwargs):
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        for idx, rand_idx in enumerate(list(rand_indices)):
            ...
            if self.hparams.save_dir is not None:
                ...
                torchaudio.save(
                    os.path.join(self.hparams.save_dir, f"{idx}-pred-{self.hparams.train_loss}-{int(prm[0]):1d}-{prm[1]:0.2f}.wav"),
