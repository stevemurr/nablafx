...
class S4Block(nn.Module):
    ...
    def forward(self, x: Tensor, conditional_information: Tensor, state=None):
        out = rearrange(x, "B H L -> B L H")
        out = self.linear(out)
        out = rearrange(out, "B L H -> B H L")

        out = self.activation1(out)
        out, _ = self.s4(out)  # S4/FFTConv returns (output, state)
        if self.batchnorm:
        ...
        return out, None  # Return output and state (state not used in this implementation)
...
class S4Model(pl.LightningModule):
    ...
    def forward(self, x: Tensor, parameters: Tensor, states: Optional[Union[List[Tensor], List[None]]] = None) -> Tensor:
        if states is None:
