...
class FiLM(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.conditional_information_adaptor = nn.Linear(conditional_information_dimension, feature_numbers * 2)

    def forward(self, x: Tensor, conditional_information: Tensor) -> Tensor:
        conditional_information = self.conditional_information_adaptor(conditional_information)
