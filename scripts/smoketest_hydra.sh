...
# 610B-MicPreamp. Each run trains for a few steps to confirm the Hydra
# entrypoint, config composition, instantiation, and GPU placement all work.
#
# Usage:
...
run_one() {
  ...
  echo ""
  echo "========== ${label} =========="
  uv run python scripts/train.py "${COMMON[@]}" "model=${model}"
}
