...
def run(
    ...
    model = GCN(**dict_args)  # create the model with args
...
if __name__ == "__main__":
    ...
    model_id = ["GCN-TF-45-S-16", "GCN-TF-45-L-16", "GCN-TF-250-S-16", "GCN-TF-250-L-16", "GCN-TF-2500-S-16", "GCN-TF-2500-L-16"]
    num_controls = [0, 0, 0, 0, 0, 0]
    num_blocks = [5, 10, 6, 11, 5, 10]
    dilation_factors = [4, 2, 4, 2, 10, 3]
    kernels = [7, 3, 11, 7, 13, 5]
    channel_width = [16, 16, 16, 16, 16, 16]
    cond_type = ["tfilm", "tfilm", "tfilm", "tfilm", "tfilm"]

    for mid, nc, b, d, k, ch, ct in zip(model_id, num_controls, num_blocks, dilation_factors, kernels, channel_width, cond_type):
    ...
    df.to_csv("tests/speed_cpu_gcn-tf.csv")
