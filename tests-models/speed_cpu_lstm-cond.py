...
if __name__ == "__main__":
    ...
    model_id = ["LSTM-32", "LSTM-C-32", "LSTM-TVC-32"]
    num_controls = [0, 4, 4]
    hidden_size = [32, 32, 32]
    num_layers = [1, 1, 1]
    cond_type = [None, "fixed", "tvcond"]

    for mid, nc, h, l, ct in zip(
        model_id,
        num_controls,
        hidden_size,
        num_layers,
        ...
        for N in frame_sizes:
            print()
            print(f"{mid}: {nc}, {h}, {l}, {ct}, frame size: {N}")
            rtf = run(
                num_controls=nc,
                hidden_size=h,
                num_layers=l,
            ...
            # if c:   mid += "-C"
            # else:   mid += "-N"
            candidates.append({"model_id": mid, "num_controls": nc, "hidden_size": h, "num_layers": l, "cond_type": ct, "rtf": rtf, "N": N})

    df = pd.DataFrame(candidates)
    df.to_csv("tests/speed_cpu_lstm-cond.csv")
