...
if __name__ == "__main__":
    ...
    model_id = ["S4-S-16", "S4-L-16"]

    for mid, b, s, ch in zip(model_id, num_blocks, s4_state_dim, channel_width):

        for N in frame_sizes:
            print()
            print(f"{mid}: {b}, {s}, {ch}, frame size: {N}")
            rtf = run(
                num_blocks=b,
                s4_state_dim=s,
                channel_width=ch,
            ...
            # if c:   mid += "-C"
            # else:   mid += "-N"
            candidates.append({"model_id": mid, "num_blocks": b, "s4_state_dim": s, "channel_width": ch, "rtf": rtf, "N": N})

    df = pd.DataFrame(candidates)
    df.to_csv("tests/speed_cpu_s4.csv")
